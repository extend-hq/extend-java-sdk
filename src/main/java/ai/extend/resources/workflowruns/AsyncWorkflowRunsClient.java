...
public class AsyncWorkflowRunsClient {
     ...
     * <p>Unlike the single <a href="https://docs.extend.ai/2026-02-09/api-reference/endpoints/workflow/create-workflow-run">Run Workflow</a> endpoint which returns the details of the created workflow runs immediately, this batch endpoint returns a <code>batchId</code>.</p>
     * <p>Our recommended usage pattern is to integrate with <a href="https://docs.extend.ai/2026-02-09/webhooks/configuration">Webhooks</a> for consuming results, using the <code>metadata</code> and <code>batchId</code> to match up results to the original inputs in your downstream systems. However, you can integrate in a polling mechanism by using a combination of the <a href="https://docs.extend.ai/2026-02-09/api-reference/endpoints/workflow/list-workflow-runs">List Workflow Runs</a> endpoint to fetch all runs via a batch, and then <a href="https://docs.extend.ai/2026-02-09/api-reference/endpoints/workflow/get-workflow-run">Get Workflow Run</a> to fetch the full outputs each run.</p>
     * <p><strong>Priority:</strong> By default, workflow runs created through this batch endpoint are assigned a priority of 90. You can override this by passing an optional <code>priority</code> value (1–100) in the request body — lower values run first.</p>
